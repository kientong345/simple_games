...
macro_rules! make_response_action {
    ($action:expr) => {
        Arc::new(tokio::sync::RwLock::new($action)) as crate::client_endpoint::HandleAction
    };
}
...
impl ResponseGetter {
    ...
    pub async fn handling_response(target: Arc<RwLock<ResponseGetter>>) -> ResponseHandler {
