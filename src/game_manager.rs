...
where A: player_manager::PlayerManager, B: room_manager::RoomManager {
    ...
    pub fn try_operate_in_room(&mut self, rid: i32) -> bool {
        ...
        // let prev_callback1 = self.player_manager.lock().unwrap().get_action_on_request(self.player1_id);
        // let prev_callback2 = self.player_manager.lock().unwrap().get_action_on_request(self.player2_id);
        // self.player_manager.lock().unwrap().set_action_on_request(self.player1_id, |msg| {});
        // self.player_manager.lock().unwrap().set_action_on_request(self.player2_id, |msg| {});

        // set rule
        match self.room_manager.lock().unwrap().get_rule_in_room(rid).unwrap() {
        ...
        let (player1_id, player2_id) = self.room_manager.lock().unwrap().get_pids_in_room(rid).unwrap();

        // set player's states
        self.player1_id = player1_id;
        let player1_connection_state = match self.player_manager.lock().unwrap().get_player_state(self.player1_id).unwrap() {
        ...
        self.player_manager.lock().unwrap().set_player_state(self.player1_id, player_manager::PlayerState::InGame(player1_connection_state));
        
        self.player2_id = player2_id;
        let player2_connection_state = match self.player_manager.lock().unwrap().get_player_state(self.player2_id).unwrap() {
            player_manager::PlayerState::Logged(state) => state,
            player_manager::PlayerState::Waiting(state) => state,
            player_manager::PlayerState::InGame(state) => state,
        };
        self.player_manager.lock().unwrap().set_player_state(self.player2_id, player_manager::PlayerState::InGame(player2_connection_state));
        ...
        self.player_manager.lock().unwrap().set_player_state(self.player1_id, player_manager::PlayerState::Waiting(player1_connection_state));
        ...
        self.player_manager.lock().unwrap().set_player_state(self.player2_id, player_manager::PlayerState::Waiting(player2_connection_state));
    ...
    pub fn execute_player_command(&mut self, cmd_code: communication::PlayerCommand) {
        match cmd_code {
    ...
    fn response_context(&mut self) {
        let game_context = GameContext {
            ...
            player1_state: self.player_manager.lock().unwrap().get_player_state(self.player1_id).unwrap(),
            player2_state: self.player_manager.lock().unwrap().get_player_state(self.player2_id).unwrap(),
        };
        let game_context_clone = game_context.clone();
        self.player_manager.lock().unwrap().response(self.player1_id, game_context.to_message_packet());
        self.player_manager.lock().unwrap().response(self.player2_id, game_context_clone.to_message_packet());
    }
}